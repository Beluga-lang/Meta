...
the exact definition of shifting arbitrary terms for later.\todo{It's now commented out -am}
...
encountered.
\unsure{Unclear; I think the point is that we might need to generate
fresh names for variables? -am}
...
\\[1em]
\infer[\EAppFnStep]{t \app s \Steps t'\;s}{t \Steps t'} \qquad
\infer[\EAppArgStep]{v \app s \Steps v\;s'}{s \Steps s' & v \Value}
\\[1em]
\infer[\EAppBeta]{(\lam x.t) \app v \Steps [v/x]t}{v \Value}
\end{array}
\]
...
\chapter{Encoding Program Transformations }
