...
\infer[u]{\Gamma \vdash \hasty x T}{u:\hasty x T \in \Gamma} \qquad
\infer[\TFn^{x,u}]{\Gamma \vdash \hasty {(\lam x.M)} {(T \arrow S)}}
                 {\Gamma, u:\hasty x T \vdash \hasty M S}\\[1em]
\infer[\TApp]{\Gamma \vdash \hasty {(M \app N)} S}
             {\Gamma \vdash \hasty M (T \arrow S)
  & \Gamma \vdash \hasty N T}
\end{array}
\]
...
\begin{theorem}
If $\proofderivc{\D}{}{M : T}$ and $\proofderiv{\St}{M \Steps N}$ then $\vdash N : T$.
\end{theorem}
\begin{proof}
By structural induction on the derivation $\proofderiv{\St}{M \Steps N}$.

\begin{case}{$\St = \ianc{\above{\V}{V \Value}}{(\lam x.M) \app V \Steps [V/x]M}{\EAppBeta}$}
...
\begin{case}{$\D = \inferaa
            ...
            { \deduce[\vspace{2pt}]{\Gamma \vdash \tmhastype M {(\tparr T S)}}{\D_1}}
            { \deduce[\vspace{2pt}]{\Gamma \vdash \tmhastype N T}{\D_2}}$}
$\C = \inferaa
            {\TApp}
            { \Gamma \vdash \tmhastype {(\tmapp M N)} S'}
            { \deduce[\vspace{2pt}]{\Gamma \vdash \tmhastype M {(\tparr {T'} S')}}{\C_1}}
            { \deduce[\vspace{2pt}]{\Gamma \vdash \tmhastype N T'}{\C_2}}$
\\[2em]

$\proofderiv{\E}{\eq {(\tparr T S)} {(\tparr T' S')}}$ \hfill by i.h. using $\D_1$ and $\C_1$ \\
$\proofderiv{\E}{\eq {(\tparr T S)} {(\tparr T S)}}$  \; and \; \emphFact{$S = S'$} \; and \; \emphFact{$T=T'$} \hfill by inversion on reflexivity.\\[1em]

Therefore there is a proof for $\eq S S'$ by reflexivity (\emphFact{since we know $S=S'$}).
\end{case}

\begin{case}{$\D = \infera{\TLam}
   {\Gamma \vdash \tmhastype {(\tmlam x T M)} {(\tparr T S)}}
...
$\proofderiv{\E}{\eq S S'}$ \hfill by i.h. using $\D_{1}$ and $\C_1$ \\
$\proofderiv{\E}{\eq S S}$ \quad and \quad \emphFact{$S = S'$} \hfill by inversion using reflexivity\\[1em]
%ause

Therefore there is a proof for $\eq {(\tparr T S)} {(\tparr T S')}$ by reflexivity.
\end{case}
...
rule \lstinline!ref!.
 ...
 %Thus, $\lstinline{M$\;$$\mdots$} = \lstinline{lam$\;$T1$\;$($\lam$x.$\;$M0$\;$$\mdots$$\;$x)}$
 ...
 %$\lstinline{M} = \lstinline{lam~T1$\;$($\lam$x.$\;$M$\;$$\mdots$x}$\lstinline{)} and
