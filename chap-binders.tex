...
$\proofderiv{\E}{\eq {(\tmarr T S)} {(\tmarr T' S')}}$ \hfill by i.h. using $\D_1$ and $\C_1$ \\
$\proofderiv{\E}{\eq {(\tmarr T S)} {(\tmarr T S)}}$  \; and \; \emphFact{$S = S'$} \; and \; \emphFact{$T=T'$} \hfill by inversion on reflexivity.\\[1em]
...
Therefore there is a proof for $\eq {(\tmarr T S)} {(\tmarr T S')}$ by reflexivity.
\end{case}
