...
In \beluga, data-types are powerful enough to encode such definitions
...
for $\tmsucc (\tmsucc \tmzero) \Value$. %\\[1em]
...
\begin{itemize}
\item How to represent inductive proofs about derivations \index{Inductive proofs about derivations} as total functions 
...
%  soundness. -bp}\todo{Problem is that there are the $value$ things, so
%  I'm not sure how to make it clean. I'll take a look. -nj}.
...
\todo{make this sentence english… ``passes to the context''? -nj}
...
The \beluga proof can seem really straightforward, but in fact there's a little
...
even more misleading\todo{Is misleading a correct word here? -nj}.

In \beluga, we will have to write this quantification explicitly. Have a look:
\todo{Please, can someone explain mlam here or under the proof? -nj}
...
For this direction, we will not be following the TAPL proof\todo{explain why}.

Let's first prove something weaker, but that can easily imply what we want to
prove:
...
\todo{bind this proof to its proposition in LaTeX? -nj}
...
You should have noticed that there's one more subcase in the \beluga code than
