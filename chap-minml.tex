...
We beging our gentle introduction mechanizing languages and proofs by
...
\begin{itemize}
\item How to represent \index{inductive proofs about derivations} as total functions 
...
type \lstinline!T! at the outside, we write in the \index{totality}
...
value and $M \Steps M'$ then we can derive \index{falsehood} (written as $\bot$).
...
\beluga; we have also included the \index{totality} declaration, stating that this
...
Note that we are \index{encoding the existential quantification} in the proposition ``there exists a value
...
(i.e. value soundness property)\todo{Remove extra checks and state value
  soundness. -bp}\todo{Problem is that there are the $num_value$ things, so
