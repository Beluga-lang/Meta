...
\mbox{Terms} & s,t & \tmfalse \bnfalt \tmtrue \bnfalt \tmif t {t_1}{t_2} \bnfalt
...
using the judgment $t \Value$. We can think of the judgment $t \Value$ as a
predicate on terms which is defined via axioms and inference rules. A
term $t$ is value iff we can give a derivation for $t \Value$.
\[
\begin{array}{c}
\multicolumn{1}{l}{\fbox{$e \Value$}: \mbox{Expression $e$ is a value}}\\[1em]
\infer[\VZero]{\tmzero \Value}{} \qquad
\infer[\VSuc]{(\tmsucc t) \Value}{t \Value} \qquad
...
We are now ready to define when a term $t$ steps using the
...
\\[1em]
\infer[\ESuc]{\tmsucc t \Steps \tmsucc t'}{t \Steps t'}\qquad
\infer[\EPred]{\tmpred t \Steps \tmpred t'}{t \Steps t'}
\\[1em]
\infer[\EIsZero]{\tmiszero t \Steps \tmiszero t'}{t \Steps t'}\qquad
\infer[\EIf]{\tmif t {t_1}{t_2} \Steps \tmif t' {t_1}{t_2}}{t \Steps t'}
\\[1em]
\multicolumn{1}{l}{\mbox{\text{Reduction Rules (Axioms)}}}
\\[1em]
\infer[\EPredZ]{\tmpred \tmzero \Steps \tmzero}{}\qquad
\infer[\EPredSucc]{\tmpred (\tmsucc t) \Steps t}{t \Value}
\\[1em]
\infer[\EIsZeroZ]{\tmiszero \tmzero \Steps \tmtrue}{} \qquad
\infer[\EIsZeroSucc]{\tmiszero (\tmsucc t) \Steps \tmfalse}{t \Value}
\\[1em]
\infer[\EIfT]{\tmif \tmtrue {t_1}{t_2} \Steps t_1}{}\qquad
\infer[\EIfF]{\tmif \tmfalse {t_1}{t_2} \Steps t_2}{}
\end{array}
\]
...
predicates and relations. To represent the judgment $t \Value$ we define a
...
The derivation &
$
\infer[\VSuc]{(\tmsucc (\tmsucc \tmzero)) \Value}
{\infer[\VSuc]{(\tmsucc \tmzero)~\Value}{
 \infer[\VZero]{\tmzero \Value}{}
 }
}
$\\[1em]
is represented as &
\lstinline!(v_s <<(suc z)>> (v_s <<z>> v_z))!\\[1em]
\end{tabular}
